...
const dt = (v?: string | null) =>
  v ? new Date(v).toLocaleString('es-CL', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—';
...
export default function VolunteerPdfDownload({ volunteer, training, equipment, attendance, records }: Props) {
  ...
  const generate = () => {
    ...
    try {
      ...
      doc.setFontSize(13);
      doc.text((currentOrg?.organization?.name ?? 'Organización').toUpperCase(), M + 3, y + 7);
      doc.setFontSize(10);
      ...
      fields.forEach((f, i) => {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7);
        doc.text(f[0].toUpperCase(), x + 2, rowY + 4);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.text(doc.splitTextToSize(f[1], colW - 4)[0] ?? '—', x + 2, rowY + 8.6);
      ...
      const stats: [string, string][] = [
        ...
        ['Registros hoja de vida', String(records.length)],
      ];
      const sw = contentW / 4;
      stats.forEach((s, i) => {
        const x = M + i * sw;
      ...
      const table = (headers: string[], widths: number[], rows: string[][], empty: string) => {
        ...
        rows.forEach(row => {
          ...
          const h = Math.max(6, Math.max(...lines.map(l => l.length)) * 3.6 + 2.6);
          ensure(h);
          let cx = M;
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(8);
      ...
      for (let p = 1; p <= pages; p++) {
        ...
        doc.setFontSize(7);
        doc.text(
          `${currentOrg?.organization?.name ?? ''} · Ficha de voluntario · Documento interno`,
          M, pageH - 6,
        );
        doc.text(`Página ${p} de ${pages}`, pageW - M, pageH - 6, { align: 'right' });
      }

      const slug = String(volunteer.name ?? 'voluntario').normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-zA-Z0-9]+/g, '-').toLowerCase();
      doc.save(`ficha-voluntario-${slug}.pdf`);
